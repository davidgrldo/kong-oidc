...
if _VERSION == "Lua 5.1" then
  load_rockspec = assert(loadfile("plugins/oidc/kong-oidc-2.0.0-1.rockspec"))
  setfenv(load_rockspec, rockspec)
else
  load_rockspec = assert(loadfile("plugins/oidc/kong-oidc-2.0.0-1.rockspec", "t", rockspec))
end
load_rockspec()
...
t.test("pins reproducible public rock source", function()
  ...
  t.equal(rockspec.source.tag, "v2.0.0")
